...
data=read.table("pre-select-KEGG.results.rf.txt",sep="\t",header=TRUE)
...
dataf$cellGroup = factor(dataf$cellGroup,levels=c(
    "CD4-Naive:NLR","CD4-Naive:LR","CD4-Activated:NLR","CD4-Activated:LR","CD8-Naive:NLR","CD8-Naive:LR","CD8-Activated:NLR","CD8-Activated:LR","CD16-NK:NLR","CD16-NK:LR","CD56-NK:NLR","CD56-NK:LR"))
...
pdf("pre.KEGG.dot.pdf",height=6,width=9,useDingbats=FALSE)
...
library(reshape2)
library(NMF)
setwd("C:/CRISPR/Endo-PD1/pseudobulk-DEG")
data=read.table("post-select-KEGG.results.rf.txt",sep="\t",header=TRUE)
data$cellGroup = paste(data$cellType,data$group,sep=":")
dataf = data[data$Pathway != "Ribosome",]
dataf$cellGroup = factor(dataf$cellGroup,levels=c(
    "CD4-Naive:NLR","CD4-Naive:LR","CD4-Activated:NLR","CD4-Activated:LR","CD8-Naive:NLR","CD8-Naive:LR","CD8-Activated:NLR","CD8-Activated:LR","CD16-NK:NLR","CD16-NK:LR","CD56-NK:NLR","CD56-NK:LR"))
...
#compress dynamic range to max abs(q) = 4
dataf[dataf$nlp < -4,]$nlp = -4
dataf[dataf$nlp > 4,]$nlp = 4
dataf$nnlp = abs(dataf$nlp)
...
pdf("post.KEGG.dot.pdf",height=6,width=9,useDingbats=FALSE)
ggplot(dataf,aes(x=cellGroup,y=Pathway,size=nnlp,color=nlp))+geom_point()+theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust=1))+scale_fill_discrete(drop=FALSE) + scale_x_discrete(drop=FALSE)+scale_color_distiller(palette="BrBG",limits=c(-4,4))
dev.off()
